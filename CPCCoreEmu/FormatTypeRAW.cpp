   ...
   case 0x01:
      {
         // size should be 8
         // Use it for check ?
         unsigned int streamPos = GetDW(file);
         unsigned int transfert_time = GetDW(file);
         stream_position_ -= 8;
   ...
   case 0x02: // Index signal data
      {
         ...
         unsigned int streamPos = GetDW(file);
         unsigned int sampleCounter = GetDW(file);
         unsigned int indexCounter = GetDW(file);
         stream_position_ -= 12;
         char buffer[256];
   ...
   case 0x03: // No more flux to transfer (one per track)
      {
         ...
         unsigned int stream_pos = GetDW(file);
         unsigned int result_code = GetDW(file);
         stream_position_ -= 8;
...
unsigned short FormatTypeRAW::GetWFromBuffer(const unsigned char* buffer, int& pos)
{
...
unsigned short FormatTypeRAW::GetW(FILE* file)
{
   unsigned short v = 0;
...
unsigned long FormatTypeRAW::GetDWFromBuffer(const unsigned char* buffer, int& pos)
{
   unsigned long v = 0;
...
unsigned long FormatTypeRAW::GetDW(FILE* file)
{
   unsigned long v = 0;
   unsigned char c[4];
   fread(&c, 4, 1, file);
...
int FormatTypeRAW::LoadDisk(std::vector<buffer_item> buffer_list, std::vector<std::string> file_list, IDisk*& created_disk, ILoadingProgress* loading_progress)
{
   ...
   int totalprogress = new_disk->m_Disk[0].NbTracks + ((new_disk->m_NbSide == 2) ? new_disk->m_Disk[1].NbTracks : 0);
   int currentprogress = 0;
   ...
   if (result == OK)
   {
      ...
      {
         for (unsigned int i = 0; i < new_disk->m_Disk[side].NbTracks; i++)
         {
            if (new_disk->m_Disk[side].Tracks[i].BitField == nullptr)
            {
               new_disk->m_Disk[side].Tracks[i].BitField = new unsigned char[DEFAULT_TRACK_SIZE * 16];
               new_disk->m_Disk[side].Tracks[i].Size = DEFAULT_TRACK_SIZE * 16;
               memset(new_disk->m_Disk[side].Tracks[i].BitField, BIT_WEAK, DEFAULT_TRACK_SIZE * 16);
            }
         }
      }
      created_disk = new_disk;
